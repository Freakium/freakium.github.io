!function(){let e=new bootstrap.Offcanvas("#itemForm"),t=4,a=flatpickr(".datepicker",{enableTime:!0,dateFormat:"Y-m-d H:i",altInput:!0,altFormat:"Y-m-d, h:i K"});function l(e,t=!1){let a=0,l={};document.getElementById("item-list").innerHTML="",e.forEach(e=>{let n=e.weight?e.price*e.weight:e.price;t&&(l.hasOwnProperty(e.type)?l[e.type]++:l[e.type]=1),m(e.id,e.name,e.type,e.colour,e.quantity,e.date,n);let i=parseFloat(e.price),r=parseFloat(e.weight),s=parseInt(e.quantity);isNaN(i)||(a+=r?i*r*s:i*s)}),t&&s(l,"All"),c(),g(a)}function n(e){let t=0;document.querySelectorAll(".badge-price").forEach(e=>{let a=parseFloat(e.getAttribute("data-price")),l=parseInt(e.getAttribute("data-quantity"));isNaN(a)||(t+=a*l)}),e&&(t+=e),g(t)}function i(e=!1){!function e(t=!1){let a=api.fetchItems();if(!a&&t){r("messageArea","A problem occurred while loading file. Please make sure you are loading the correct inventory file.","danger",3);return}l(a,!0)}(e);let t;t=api.fetchTitle()??"Inventory",document.getElementById("inventoryTitle").value=t;let a;a=api.fetchCurrency()??"$",document.getElementById("currencySymbolSelect").value=a,u(a)}function r(e,t,a,l){if("string"!=typeof t||"string"!=typeof e||0==e.length)return;if(0===t.length){document.getElementById(e).innerHTML="<br>";return}let n=`<div class="alert alert-${a} alert-dismissible fade show d-flex shadow py-2" role="alert">
      ...
      </div>`;document.getElementById(e).innerHTML=n,l&&setTimeout(()=>{document.getElementById(e).innerHTML="<br>"},1e3*parseInt(l))}function s(e,t){let a='<button class="btn btn-sm btn-outline-primary" id="typeListBtn-All" data-bs-toggle="button" onclick="typeFilter(this)">All</button>',l=Object.keys(e);l.sort(),l.forEach(t=>{a+=`<button class="btn btn-sm btn-outline-primary typeListBtn" id="typeListBtn-${t}" data-count="${e[t]}" data-bs-toggle="button" onclick="typeFilter(this)">${t}</button>`}),document.getElementById("type-list").innerHTML=a;let n=document.getElementById(`typeListBtn-${t}`);n?n.click():document.getElementById("typeListBtn-All").click()}function d(e,t,a){let l={},n="";switch(document.querySelectorAll(".typeListBtn").forEach(e=>{l[e.innerHTML]=parseInt(e.getAttribute("data-count")),e.classList.contains("active")&&(n=e.innerHTML)}),e){case"delete":l.hasOwnProperty(t)&&(l[t]--,0===l[t]&&delete l[t]);break;case"update":l.hasOwnProperty(a)&&(l[a]--,0===l[a]&&delete l[a]);case"add":l.hasOwnProperty(t)?l[t]++:l[t]=1}s(l,n)}function m(e,t,a,l,n,i,r){let s=new Date(i),d=s.toLocaleString("en-US",{dateStyle:"long",timeStyle:"short"}),m=`<span class="currency-display" data-price="${r}">${y(r??0)}</span>`;document.getElementById("item-list").innerHTML+=`<div class="col" id="${e}">
